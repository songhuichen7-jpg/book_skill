...
- **生图失败时 STOP，绝不静默降级**：生图脚本会打印 codex 退出码/报错。一旦报错或零产出（常见根因：`~/.codex/config.toml` 的 `service_tier` 被当前 codex CLI 拒绝、未登录），**停下、把错误告诉用户、先修前置条件再继续**——绝不"取最新 png"复用旧图、绝不用本机代码画裸方/圆/三角顶替扉页插画、绝不用简单 `#flow` 悄悄顶替该生成的彩色信息图。扉页是**图像模型手绘的概念线描**，不是程序几何形。详见 `references/illustrations.md` 顶部红线。
